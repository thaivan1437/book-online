...
/* 
Ham nay se dua css cua file dat trong folder: css vao
Tham so: $name: la ten file css (ko bao gom duoi .css va ko can duong dan)
*/
...
function isAuthor($idPost) {
    global $d, $com;
    $d->reset();
    $d->setTable($com);
    $d->setWhere('id', $idPost);
    $d->select();
    $row = $d->fetch_array();
    if ($row['author']==$_SESSION['login']['username']) {
        return true;
    } else {
        return false;
    }

...
if (isset($_POST['continue'])) {
	...
	$pttt = $_POST['pttt'];
	$ptvc = $_POST['ptvc'];
	$idTinh = $_POST['province_chon'];
	$idHuyen = $_POST['district_chon'];
	$idXa = $_POST['district_chon2'];

	$d->reset();
	$sql="select ten from #_place_city where id='".$idTinh."'";
	$d->query($sql);
	$theResult=$d->fetch_array();
	$tinh = $theResult['ten'];

	$d->reset();
	$sql="select ten from #_place_dist where id='".$idHuyen."'";
	$d->query($sql);
	$theResult=$d->fetch_array();
	$huyen = $theResult['ten'];

	$d->reset();
	$sql="select ten from #_place_ward where id='".$idXa."'";
	$d->query($sql);
	$theResult=$d->fetch_array();
	$xa = $theResult['ten'];
	...
	if($_SESSION["couple"]["gia_vi"]!=''){
		...
		// thiết lập Đơn hàng
		$donhang.='<div>';
		$donhang.='<table class="table table-bordered" border="0" cellpadding="5px" cellspacing="1px" style="font-size:13px;">';
	...
	/* if ($httt == 'thanh-toan-bao-kim') {  //thanh toán qua Bảo Kim
		...
		if(!empty($_SESSION['login_web'])) {
			$sql = "INSERT INTO  table_donhang (madonhang,hoten,dienthoai,diachi,email,pttt,ptvc,tinh,huyen,xa,tonggia,noidung,donhang,ngaytao,trangthai,noinhan,loaithanhtoan,dienthoainhan, tennhan,id_dangnhap ) 
				VALUES ('$mahoadon','$hoten','$dienthoai','$diachi','$email','$pttt','$ptvc','$tinh','$huyen','$xa','$tonggia','$noidung','$donhang','$ngaytao','1','$diachinhan','0','$dienthoainhan','$hotennhan', '".$_SESSION["login_web"]["id"]."')";
			
			mysql_query($sql) or die(mysql_error());
		} else {
			$sql = "INSERT INTO  table_donhang (madonhang,hoten,dienthoai,diachi,email,pttt,ptvc,tinh,huyen,xa,tonggia,noidung,donhang,ngaytao,trangthai,noinhan,loaithanhtoan,dienthoainhan,tennhan ) 
				VALUES ('$mahoadon','$hoten','$dienthoai','$diachi','$email','$pttt','$ptvc','$tinh','$huyen','$xa','$tonggia','$noidung','$donhang','$ngaytao','1','$diachinhan','0','$dienthoainhan','$hotennhan')";
		...
		for ($i = 0; $i < $max; $i++) {
			...
			$data["id_sp"]=$pid;
			$data["soluong"]=$q;
			$data["size"]=$_SESSION['cart'][$i]['size'];
			$data["color"]=$_SESSION['cart'][$i]['color'];
			$data["id_dh"]=$iduser;
			$d->reset();
			$d->setTable("dhct");
			$d->insert($data);
		...
		for ($i = 0; $i < $max; $i++) {
			...
			$body.='<tr>
					...
					<td style="text-align:center;width:15%;">' . number_format(get_price($pid), 0, ',', '.') . '&nbsp;VNĐ ( Nếu giá = 0 nghĩa là GIÁ : LIÊN HỆ )</td>
					<td style="text-align:center; width:20%;">' . number_format(get_price($pid) * $q, 0, ',', '.') . '&nbsp;VNĐ ( Nếu giá = 0 nghĩa là GIÁ : LIÊN HỆ )</td>
				</tr>';
		}
		$body.='</table>';
			if($_SESSION["shipping"]>0){
			...
			if($httt==1) {
				$body.='<div>Hình thức thanh toán: thanh toán tại cửa hàng.</div>';
			} else if($httt==2) {
				$body.='<div>Hình thức thanh toán: thanh toán khi nhận được hàng.</div>';
			} else if($httt==3) {
				$body.='<div>Hình thức thanh toán: thanh toán qua chuyển khoản ngân hàng.</div>';
			}
			$body.='<div>Phương thức vận chuyển: '.$ptvc.'</div>';
			$body.='<div>Phương thức thanh toán: '.$pttt.'</div>';
			$body.='<div>Thanh toán : ' . number_format(get_order_total(), 0, ',', '.') . '&nbsp;VNĐ</div>';
			
		$body.='Mọi yêu cầu giúp đỡ xin liên hệ mail:'.$row_setting['email'].'  <br />
		...
		$mail->AddAddress("info@derladie.vn", $_POST['hoten']);
...
$d->reset();
$sql = "select tenkhongdau, ten_$lang as ten, photo, noidung_$lang as noidung from #_about where type='thanh-toan' and hienthi=1 order by id";
$d->query($sql);
$pt_thanhtoan = $d->result_array();

$d->reset();
$sql = "select tenkhongdau, ten_$lang as ten, photo, noidung_$lang as noidung from #_about where type='van-chuyen' and hienthi=1 order by id";
$d->query($sql);
$pt_vanchuyen = $d->result_array();

$title_tcat = _thanhtoan;

...
$urldanhmuc.= (isset($_REQUEST['id_parents'])) ? "&id_parents=" . addslashes($_REQUEST['id_parents']) : "";
        ...
        if($multi=='del'){
            for($i=0;$i<$count;$i++){
                ...
                if(mysql_query($sql)){
                    ...
                    //Xóa tất cả size kem theo
                    $sql = "select * from #_product_size where id_item= ".$id_array[$i]."";
                    $d->query($sql);
                    $rs=$d->result_array();
                    foreach($rs as $v){
                        $sql = "delete from #_product_size where id='" . $v["id"] . "'";
                        $d->query($sql);
                    }
                    //Xóa tất cả color kem theo
                    $sql = "select * from #_product_color where id_item= ".$id_array[$i]."";
                    $d->query($sql);
                    $rs=$d->result_array();
                    foreach($rs as $v){
                        $sql = "delete from #_product_color where id='" . $v["id"] . "'";
                        $d->query($sql);
                    }
                }
...
function save_item() {
    ...
    $data['h2'] = $_POST['h2'];
    $data['h3'] = $_POST['h3'];
    $data['tag_vi'] = magic_quote($_POST['tag_vi']);
    $data['tag_en'] = magic_quote($_POST['tag_en']);
    $data['tag_slug_vi'] = $_POST['tag_slug_vi'];
    $data['tag_slug_en'] = $_POST['tag_slug_en'];
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->num_rows() > 0) {
            while ($row = $d->fetch_array()) {
                ...
                //Xóa tất cả size kem theo
                $sql = "select * from #_product_size where id_item= ".$row['id']."";
                $d->query($sql);
                $rs=$d->result_array();
                foreach($rs as $v){
                    $sql = "delete from #_product_size where id='" . $v["id"] . "'";
                ...
                //Xóa tất cả color kem theo
                $sql = "select * from #_product_color where id_item= ".$row['id']."";
                $d->query($sql);
                $rs=$d->result_array();
                foreach($rs as $v){
                    $sql = "delete from #_product_color where id='" . $v["id"] . "'";
...
function get_lists() {
    global $d, $items, $paging, $type, $subcat,$urldanhmuc;
    #----------------------------------------------------------------------------------------
    if(!empty($_POST)){
        $multi=$_REQUEST['multi'];
        $id_array=$_POST['iddel'];
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "default.php?com=product&act=man_list&type=".$type."&subcat=".$subcat.$urldanhmuc;
    $maxR = 15;
    $maxP = 10;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function save_list() {
    ...
    $data['noidung_jp'] = magic_quote($_POST['noidung_jp']);
    $data['noidung_vi'] = magic_quote($_POST['noidung_vi']);
    $data['noidung_en'] = magic_quote($_POST['noidung_en']);
    $data['mota_vi'] = magic_quote($_POST['mota_vi']);
    $data['mota_en'] = magic_quote($_POST['mota_en']);
    $data['mota_jp'] = magic_quote($_POST['mota_jp']);
    $data['com'] = $subcat;
    $data['type'] = $type;
    $data['h1'] = $_POST['h1'];
    $data['h2'] = $_POST['h2'];
    ...
    if ($id) {
        ...
        if ($photo1 = upload_image("file1", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_product, $file_name1)) {
            ...
            if ($d->num_rows() > 0) {
                $row = $d->fetch_array();
                delete_file(_upload_product . $row['avata']);
            }
        }

...
function save_item() {
    ...
    if ($id) {
        ...
        if ($thumb = upload_image("file2", 'jpg|png|gif', _upload_tintuc, $file_name . '2')) {
            $data['thumb'] = $thumb;
            $d->setTable('news');
            $d->setWhere('id', $id);
            $d->select();
...
function get_cats() {
    ...
    if(!empty($_POST)){
        ...
        if($multi=='show'){
            ...
            redirect("default.php?com=news&act=man_cat&type=".$type);          
        }
        ...
        if($multi=='del'){
            for($i=0;$i<$count;$i++){
                ...
                $sql = "select id, photo from #_news_item where id= ".$id_array[$i]."";
                $d->query($sql);
                if($d->num_rows()>0){
                    while($row = $d->fetch_array()){
                        delete_file(_upload_tintuc.$row['photo']);         

...
<form name="supplier" id="validate" class="form" action="default.php?com=order&act=save" method="post" enctype="multipart/form-data">
	<div class="widget">
		...
		<div class="formRow">
			<label>Giao hàng tận nơi tại huyện / quận</label>
			<div class="formRight">
             <?=@$item['huyen']?>
			</div>
			<div class="clear"></div>
		...
		<div class="formRow">
			<label>Giao hàng tận nơi tại xã / phường</label>
			<div class="formRight">
             <?=@$item['xa']?>
			</div>
			<div class="clear"></div>
	...
	<div class="widget">
        ...
        <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
			...
			<?php
				...
				$size=$result_ctdonhang[$i]['size'];
				$color=$result_ctdonhang[$i]['color'];
			...
			?>
				<tr id="productct<?=$result_ctdonhang[$i]['id']?>">
					...
					<div id="ajaxloader"><img class="numloader" id="ajaxloader<?=$result_ctdonhang[$i]['id']?>" src="images/loader.gif" alt="loader" /></div>
						&nbsp;</td>
					<?php if(!empty($size)){ ?>
					<td align="center" id="id_price<?=$result_ctdonhang[$i]['id']?>"><?=number_format(get_price_size($size)*$result_ctdonhang[$i]['soluong'],0, ',', '.')?>&nbsp;VNĐ</td>
					<?php } else { ?>
